...
//! Windows guest running inside a KVM virtual machine on an ARM64 host.
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let kernel_info = {
        ...
        // On ARM64 architecture, the kernel is usually found using the
        // `VBAR_EL1` register, which contains the base address of the
        // exception vector table. This register is set by the operating
        // system during boot and is left unchanged, so it seeds a backward
        // scan to locate the kernel image base.
        //
        // Therefore, we can take an arbitrary register set at any point in
        // time (as long as the OS has booted and the page tables are set up)
        // and use them to find the kernel.
