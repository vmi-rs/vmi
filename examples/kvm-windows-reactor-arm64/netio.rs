    ...
    Driver::Architecture: ArchAdapter<Driver>,
{
    ...
    // the calling convention: on AAPCS64 it sets x0 = value and PC = x30
    // (LR) with no stack-pointer fixup, unlike the amd64 path that pops the
    // pushed return address off the stack.
    let registers = vmi.registers().return_from_function(vmi.core(), 0)?;
