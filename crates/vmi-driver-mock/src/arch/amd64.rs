//! In-memory AMD64 mock [`VmiDriver`] for testing.
//!
...
pub enum Call {
    ...
    AllocateGfn {
        /// Frame that was handed out.
        gfn: Gfn,
    },

    /// `change_view_gfn(view, old, new)`.
    ChangeViewGfn {
        /// View whose mapping changed.
        view: View,
    ...
    ResetViewGfn {
        /// View whose mapping was cleared.
        view: View,

        /// Frame whose remapping was removed.
        gfn: Gfn,
    },

    /// `set_memory_access(gfn, view, access)`.
    SetMemoryAccess {
        /// Frame whose protection changed.
        gfn: Gfn,

        /// View whose mapping carries the new protection.
        view: View,
...
impl MockDriver {
    ...
    pub fn fill_page(&self, gfn: Gfn, byte: u8) {
    ...
    pub fn write_original(&self, gfn: Gfn, offset: usize, bytes: &[u8]) {
    ...
    pub fn access(&self, gfn: Gfn, view: View) -> MemoryAccess {
        self.access
            .borrow()
            .get(&(view, gfn))
            .copied()
    ...
    pub fn set_initial_access(&self, gfn: Gfn, view: View, access: MemoryAccess) {
        self.access.borrow_mut().insert((view, gfn), access);
    }

    /// Writes a page table entry at the given physical address.
    pub fn write_pte(&self, pa: Pa, pte: PageTableEntry) {
    ...
    pub fn arm_fault(&self, op: Op, nth: usize) {
...
mod tests {
    ...
    fn read_page_is_recorded() {
        ...
        assert_eq!(driver.calls(), vec![Call::ReadPage { gfn: Gfn(0x10) }]);
        assert_eq!(driver.count(|c| matches!(c, Call::ReadPage { .. })), 1);
    }
    ...
    fn armed_fault_fires_on_the_nth_call_only_and_is_not_recorded() {
        ...
        assert_eq!(driver.count(|c| matches!(c, Call::ReadPage { .. })), 2);
    }
    ...
    fn allocate_gfn_hands_out_increasing_zeroed_frames() {
        ...
        assert_eq!(
            driver.calls(),
            vec![
                Call::AllocateGfn { gfn: first },
                Call::AllocateGfn { gfn: second }
            ]
        );
