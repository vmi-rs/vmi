...
use vmi_core::{
    ...
    VmiEvent, VmiEventFlags,
    arch::{EventInterrupt as _, EventReason as _},
};
use vmi_driver_mock::arch::amd64::{Call, MockDriver, Op};

use super::super::{
    Breakpoint, BreakpointController, BreakpointManager, MemoryController, TapController,
};
...
/// Guest code page that receives the test breakpoints.
...
#[test]
fn pending_same_ctx_different_tag_keeps_the_first_tag() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    let ctx = ctx_at(OFFSET, ROOT1);
    // A key claims a pending address once, so a second tag at the same context
    // is a repeated claim rather than a new entry.
    ...
    // The claim keeps the tag it was made with, as activating it shows.
    let pa = pa_at(CODE_GFN, OFFSET);
    ...
    let breakpoint = manager.get_by_event(&event, ()).expect("breakpoint");
    assert_eq!(breakpoint.tag(), "a");

    Ok(())
}

#[test]
fn pending_global_conflict_is_rejected() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    let ctx = ctx_at(OFFSET, ROOT1);
    // A key claims the pending address as non-global.
    assert!(manager.insert_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_tag("a"), None)?);

    // Reinterpreting that claim as global is rejected, just as on the active
    // side, so the conflict cannot reach the page-in that restores it.
    let result = manager.insert_with_hint(
        &vmi,
        Breakpoint::new(ctx, VIEW).global().with_tag("b"),
        None,
    );
    assert!(matches!(result, Err(VmiError::Other(_))));

    // The rejected insertion leaves the claim intact, so activating it installs
    // the non-global breakpoint it was made with.
    let pa = pa_at(CODE_GFN, OFFSET);
    let update = PageEntryUpdate {
        view: VIEW,
        ctx,
    ...
    let own_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT1);
    let breakpoint = manager.get_by_event(&own_root, ()).expect("breakpoint");
    assert_eq!(breakpoint.tag(), "a");
    assert!(!breakpoint.global());

    let foreign_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT2);
    assert!(!manager.contains_by_event(&foreign_root, ()));
...
#[test]
fn active_same_ctx_different_tag_keeps_the_first_tag() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();
    ...
    // A second tag at the same (key, context) is a repeated claim on an address
    // this key already holds, so it installs nothing and the tag is discarded.
...
#[test]
fn remove_tears_down_active_when_a_pending_copy_shares_the_va() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    // ROOT1 becomes the canonical root for the global VA and is installed.
    let pa = pa_at(CODE_GFN, OFFSET);
    assert!(manager.insert_with_hint(&vmi, bp_global(OFFSET, ROOT1, VIEW), Some(pa))?);

    // The same global VA has no translation under ROOT2 yet, so it lands in the
    // pending map under the context the caller registered, not the canonical
    // one. Both copies of the one global breakpoint now exist at once.
    assert!(manager.insert_with_hint(&vmi, bp_global(OFFSET, ROOT2, VIEW), None)?);
    rec_clear_log();

    // Removing with ROOT2 clears the pending copy and must go on to tear down
    // the active copy folded onto ROOT1, not report success and leak it.
    assert!(manager.remove_with_hint(&vmi, bp_global(OFFSET, ROOT2, VIEW), Some(pa))?);
    assert_eq!(rec_count(is_remove), 1);
...
#[test]
fn page_out_then_page_in_keeps_the_claim() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();
    ...
    manager.insert_with_hint(
        &vmi,
        Breakpoint::new(ctx, VIEW).global().with_tag("a"),
        Some(pa),
    )?;
    ...
    // The restored breakpoint is the one that was claimed, tag and global flag
    // included, so a foreign root matches it again.
    let foreign_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT2);
    let breakpoint = manager.get_by_event(&foreign_root, ()).expect("breakpoint");
    assert_eq!(breakpoint.tag(), "a");
    assert!(breakpoint.global());
...
#[test]
fn get_by_event_returns_the_matching_breakpoint() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();
...
#[test]
fn global_breakpoint_is_removed_by_a_non_canonical_root() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    // The first root registered for the VA becomes the canonical one.
    let pa = pa_at(CODE_GFN, OFFSET);
    assert!(manager.insert_with_hint(&vmi, bp_global(OFFSET, ROOT1, VIEW), Some(pa))?);
    assert!(!manager.insert_with_hint(&vmi, bp_global(OFFSET, ROOT2, VIEW), Some(pa))?);
    rec_clear_log();

    // The second root names the same global breakpoint, so removing with it
    // tears the breakpoint down.
    assert!(manager.remove_with_hint(&vmi, bp_global(OFFSET, ROOT2, VIEW), Some(pa))?);
    assert_eq!(rec_count(is_remove), 1);
    assert_eq!(rec_count(is_unmonitor), 1);
...
#[test]
fn distinct_keys_at_one_address_are_independent() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = BreakpointManager::<RecordingController, u32>::new();

    let ctx = ctx_at(OFFSET, ROOT1);
    let pa = pa_at(CODE_GFN, OFFSET);
    // Two owners claim the same address, which is how one address carries two
    // breakpoints that can be removed independently.
    assert!(manager.insert_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_key(1u32), Some(pa))?);
    assert!(manager.insert_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_key(2u32), Some(pa))?);
    assert_eq!(rec_count(is_insert), 2);
    assert_eq!(rec_count(is_monitor), 1);

    // Releasing one claim leaves the other one installed.
    rec_clear_log();
    assert!(manager.remove_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_key(1u32), Some(pa))?);
    assert_eq!(rec_count(is_remove), 1);
    assert_eq!(rec_count(is_unmonitor), 0);
    assert!(!manager.contains_by_address(ctx, 1u32));
    assert!(manager.contains_by_address(ctx, 2u32));

    // Releasing the last claim tears the page down.
    assert!(manager.remove_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_key(2u32), Some(pa))?);
    assert_eq!(rec_count(is_unmonitor), 1);
    assert!(!manager.contains_by_address(ctx, 2u32));
...
#[test]
fn global_breakpoint_onto_a_non_global_context_is_rejected() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    let ctx = ctx_at(OFFSET, ROOT1);
    let pa = pa_at(CODE_GFN, OFFSET);
    // A non-global breakpoint claims the (key, context) first.
    manager.insert_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_tag("a"), Some(pa))?;
    rec_clear_log();

    // The claim cannot be reinterpreted as global, since that would change when
    // the installed breakpoint matches.
    let result = manager.insert_with_hint(
        &vmi,
        Breakpoint::new(ctx, VIEW).global().with_tag("b"),
        Some(pa),
    ...
    // The rejected insertion leaves the existing claim untouched, so the
    // address still matches its own root only.
    assert!(rec_calls().is_empty());
    let own_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT1);
    let foreign_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT2);
    assert!(manager.contains_by_event(&own_root, ()));
    assert!(!manager.contains_by_event(&foreign_root, ()));
...
#[test]
fn non_global_breakpoint_onto_a_global_context_is_rejected() -> Result<(), VmiError> {
    let vmi = make_vmi(MockDriver::new())?;
    let mut manager = RecManager::new();

    let ctx = ctx_at(OFFSET, ROOT1);
    let pa = pa_at(CODE_GFN, OFFSET);
    // A global breakpoint claims the (key, context) first.
    manager.insert_with_hint(
        &vmi,
        Breakpoint::new(ctx, VIEW).global().with_tag("a"),
        Some(pa),
    ...
    // Narrowing that claim to a single root is rejected as well.
    let result = manager.insert_with_hint(&vmi, Breakpoint::new(ctx, VIEW).with_tag("b"), Some(pa));
    assert!(matches!(result, Err(VmiError::Other(_))));

    // The global claim survives, so a foreign root still matches.
    assert!(rec_calls().is_empty());
    let foreign_root = bp_event(Some(VIEW), CODE_GFN, va_at(OFFSET), ROOT2);
    assert!(manager.contains_by_event(&foreign_root, ()));
